...
public class ResumoDAO {
	...
	public List<Double> atualizarMes(String data) throws SQLException{
		...
		try {
			...
			st = conn.prepareStatement("SELECT * FROM rendimento WHERE (YEAR(STR_TO_DATE(data, '%m/%Y')) = ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) <= ? AND mensal > 0) "
			...
			st = conn.prepareStatement("SELECT * FROM investimentos WHERE (YEAR(STR_TO_DATE(data, '%m/%Y')) = ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) <= ? AND mensal > 0) "
			...
			st = conn.prepareStatement("SELECT * FROM fundo WHERE (YEAR(STR_TO_DATE(data, '%m/%Y')) = ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) <= ? AND mensal > 0) "
										+ "OR (YEAR(STR_TO_DATE(data, '%m/%Y')) < ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) >= ? AND mensal > 0) OR (STR_TO_DATE(data, '%m/%Y') = STR_TO_DATE(?, '%m/%Y') AND mensal > 0)");
			st.setInt(1, Integer.parseInt(date[1]));
			st.setInt(2, Integer.parseInt(date[0]));
			st.setInt(3, Integer.parseInt(date[1]));
			...
			st = conn.prepareStatement("SELECT * FROM despesas WHERE (YEAR(STR_TO_DATE(data, '%m/%Y')) = ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) <= ? AND mensal > 0) "
										+ "OR (YEAR(STR_TO_DATE(data, '%m/%Y')) < ? AND MONTH(STR_TO_DATE(data, '%m/%Y')) >= ? AND mensal > 0) OR (STR_TO_DATE(data, '%m/%Y') = STR_TO_DATE(?, '%m/%Y') AND mensal > 0)");
			st.setInt(1, Integer.parseInt(date[1]));
			st.setInt(2, Integer.parseInt(date[0]));
			st.setInt(3, Integer.parseInt(date[1]));
